...
import { batchUpdate, loadVideoList, openBehaviour } from 'src/modules/redux/video-state/actions';
...
export class VideoConnector implements IService {
  ...
  private videoStateSnapshot: VideoState;
  ...
  set loadingStateBroadcast(value: RequestStateEnum) {
  ...
  private set loadingState(value: RequestStateEnum) {
    this.videoStateSnapshot.loadingState = value;
  }

  private get videos(): VideoModel[] {
    return this.videoStateSnapshot.videoList;
  }
  private set videos(value: VideoModel[]) {
    this.videoStateSnapshot.videoList = value;
  }

  private get nextPageToken(): string {
    return this.videoStateSnapshot.nextPageToken;
  }
  private set nextPageToken(value: string) {
    this.videoStateSnapshot.nextPageToken = value;
  }

  private get remainingVideos(): number {
    return this.videoStateSnapshot.remainingVideos;
  }
  private set remainingVideos(value: number) {
    this.videoStateSnapshot.remainingVideos = value;
  }
  ...
  loadVideos(isNextPage: boolean = false): void {
    this.loadingStateBroadcast = RequestStateEnum.loading;
    this.startBatch();
  ...
  private startBatch(): void {
    this.videoStateSnapshot = Object.assign({}, this.videoState);
  }

  private commitChanges(): void {
    this.dispatch(batchUpdate(this.videoStateSnapshot));
    // to be safe from unwanted direct update
    this.videoStateSnapshot = Object.assign({}, this.videoStateSnapshot);
  }
}
